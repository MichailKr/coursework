...
def runGame(screen, screen_size, grid_size, side_length, mode):  # Теперь принимаем screen и screen_size
    ...
    # screen = pygame.display.set_mode(size) # Не создаем новое окно, используем переданное
    pygame.display.set_caption("\"Esc\" to exit")
    # Меняем размер существующего окна
    # Нужно определить размер окна для игры. Он может отличаться от меню.
    # Посчитаем размер исходя из grid_size и side_length
    game_screen_width = grid_size * (side_length + border_width) + border_width
    game_screen_height = grid_size * (side_length + border_width) + border_width
    if mode == 4:
...
if __name__ == "__main__":
    ...
    while Run:
        if current_state == states[0]:  # Главное меню
            ...
            Run, grid_size, side_length, mode, next_state = ui_file.startScreen(screen,
                                                                                screen_size)  # Передаем screen и screen_size
